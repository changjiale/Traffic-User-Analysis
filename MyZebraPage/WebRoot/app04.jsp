	...
	<div class="box">
		...
		<div class="middle">
			...
			<ul class="list_two clear">
				...
				<li class="list_on01"><a href="${pageContext.request.contextPath }/app03.jsp">网站表现</a>
				</li>
				<li><a href="#">小区上网能力</a>
				</li>
				<li><a href="#">小区上网喜好</a>
						...
						<table width="729" border="0" cellspacing="0" cellpadding="0" class="table_list">
							<tr>
								...
								<td width="274" align="left" valign="middle">
									<c:if test="${param.type==1 }">尝试次数</c:if>
									<c:if test="${param.type==2 }">接受次数</c:if>
									<c:if test="${param.type==3 }">总流量</c:if>
									<c:if test="${param.type==4 }">上行流量</c:if>
									<c:if test="${param.type==5 }">下行流量</c:if>
								</td>
								<td width="50"> 
								</td>
							</tr>
							...
							<tr>
								<td height="147" colspan="5" align="center" valign="middle">网站一天折线图</td>
							</tr>
						</table>
					</form>
				</div>
